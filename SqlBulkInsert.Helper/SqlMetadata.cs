    ...
    public class SqlMetadata<T>
    {
        ...
        public SqlMetadata()
        {
            ...
            if (GeneratedIdProperty != null && ContainerIdProperties == null)
                throw new InvalidOperationException("A dto type that has a generatedId field must have a containerId field to be bulk inserted.");
            if (TreeParentIdProperty != null && GeneratedIdProperty == null)
                throw new InvalidOperationException("A dto type that has a treeParentId field must have a generatedId field to be bulk inserted.");
        }
        ...
        private static ColumnProperty CreateColumnProperty<TColumnType>(PropertyInfo property)
            where TColumnType : Attribute, IColumnAttribute
        {
            var columnAttr = (IColumnAttribute)property.GetCustomAttributes(typeof(TColumnType), true).FirstOrDefault();
